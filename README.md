...
- Per active tab:
  ...
  - **Completeness bar**: how much of the tab was actually *looked at* — any non-blank status counts, including Missing, which is a finding rather than a gap. Deliberately a different number from Accounted: a yard can be 100% walked and 96% accounted for.
  - **Breakdown table**: grouped by Equipment Class (assets), MFR (parts), or Bin Location (bulk). Sorted and emphasised by **% Issues** (missing + damaged) rather than % missing alone, so the column carries signal on a yard where nothing is missing but things are damaged. Columns that are empty for every group are dropped rather than left blank. Qty-weighted for parts/bulk so 90-of-100 missing reads heavier than 1-of-1. **Discrepancy-focused** — groups that are fully accounted for (nothing missing/damaged/short/over and everything counted) are hidden by default; a "*N fully-accounted groups hidden · Show all*" toggle reveals them.
